...
function installing_prereg() {
  ...
  helm -n $NS install prereg-application mosip/prereg-application --set image.repository=technogovstack/pre-registration-application-service --set image.tag=release-1.2.0.1.1 --version $CHART_VERSION

  echo Installing prereg-booking
  helm -n $NS install prereg-booking mosip/prereg-booking --set image.repository=technogovstack/pre-registration-booking-service --set image.tag=release-1.2.0.1.1 --version $CHART_VERSION
  ...
  helm -n $NS install prereg-ui mosip/prereg-ui --set prereg.apiHost=$PREREG_HOST --set image.repository=technogovstack/pre-registration-ui --set image.tag=release-1.2.0.1.1 --version $CHART_VERSION
